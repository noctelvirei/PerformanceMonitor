...
The Settings page can discover SQL Server instances through dbatools. The central repository service starts PowerShell on the monitoring server and imports dbatools there; it does not install anything or run commands on the monitored SQL Servers. Install dbatools for the same Windows account that runs the central repository service, or install it for all users on the monitoring server. If dbatools is missing, Discovery fails during preflight with an install message.

Use **Discovery** to preview SQL Servers from registered servers, named targets, Active Directory SQL SPNs, AD server/computer searches, SQL Browser enumeration, or an IP range. Discovery runs as a tracked background job; the page shows the resolved plan, PowerShell/dbatools startup, elapsed wait messages, per-phase timeout, and final result instead of leaving the browser silent. Discovered instances are shown as candidates; choose **Add** or **Add all shown** to turn them into normal monitored server entries.
...
- `Discovery Mode`: choose one source first: registered servers, known servers, registered SQL SPNs, AD Windows servers, AD computers, IP range, or SQL Browser broadcast. The page changes the available options to match that source.
- `Registered servers`: runs dbatools `Get-DbaRegServer` and previews the returned list without testing SQL login. Leave CMS SQL Instance blank to read local SSMS/Azure Data Studio registrations for the service account, or set it to a Central Management Server such as `mylaptop`. Set Registered Groups to values such as `dev` or `Production\Database Servers`. Name Patterns are dbatools regex patterns matched against registered `Name` and `ServerName`, such as `^(dvw|dnx)`.
...
- `SQL Browser broadcast`: uses DataSourceEnumeration, which is the same style of UDP broadcast enumeration used by SSMS. dbatools uses those results to compile candidate computers, so keep the Browser scan enabled if you want matching browser details in the scan phase. If your estate disables SQL Browser, use registered servers, SPNs, DomainServer, or TCPPort scanning instead.
...
- `Scan Method`: exposes dbatools `Find-DbaInstance -ScanType` choices including TCPPort, SQLService, DNSResolve, Ping, SPN, Browser, SqlConnect, Default, and All. Browser stays available for estates that use it, but TCPPort is the default because many environments disable SQL Browser.
