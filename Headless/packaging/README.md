...
- `InstallHeadlessService-WindowsAuth.cmd` if the monitor should connect to SQL Servers with Windows authentication. It prompts for the Windows account that will run the service.

The installer starts the `PerformanceMonitorHeadless` Windows service and opens:
...
Run `UninstallHeadlessService.cmd` as an administrator. Local settings and collected data are left in place unless you run `uninstall-headless-service.ps1 -RemoveLocalData`.
