...
public sealed class SqlServerCentralRepositoryStore : ICentralRepositoryStore
{
    private readonly IOptionsMonitor<MonitorOptions> _options;
    private readonly ILogger<SqlServerCentralRepositoryStore> _logger;
    private readonly SemaphoreSlim _writeLock = new(1, 1);
    private static long s_idCounter = DateTime.UtcNow.Ticks;

    public SqlServerCentralRepositoryStore(
        IOptionsMonitor<MonitorOptions> options,
        ILogger<SqlServerCentralRepositoryStore> logger)
    {
        _options = options;
        _logger = logger;
    }
...
SELECT
    ...
    (
        ...
        WHERE latest.rn = 1
        AND   latest.status IN (N'ERROR', N'AUTH_FAILED', N'PERMISSIONS')
    ...
    (
        ...
        WHERE latest.rn = 1
        AND   latest.status IN (N'ERROR', N'AUTH_FAILED', N'PERMISSIONS')
        ORDER BY CASE WHEN latest.status IN (N'ERROR', N'AUTH_FAILED') THEN 1 ELSE 2 END, latest.collection_time DESC
    ) AS recent_alert,
    (
        SELECT TOP (1) CASE WHEN latest.status IN (N'ERROR', N'AUTH_FAILED') THEN N'red' ELSE N'yellow' END
        FROM
        (
            SELECT
                cl.status,
...
SELECT
    ...
    lc.collector_name,
    CASE WHEN lc.status IN (N'ERROR', N'AUTH_FAILED') THEN N'red' ELSE N'yellow' END AS severity,
...
WHERE lc.rn = 1
AND   lc.status IN (N'ERROR', N'AUTH_FAILED', N'PERMISSIONS')
AND   COALESCE(s.is_enabled, CONVERT(bit, 1)) = CONVERT(bit, 1)
ORDER BY
    CASE WHEN lc.status IN (N'ERROR', N'AUTH_FAILED') THEN 1 ELSE 2 END,
